...
// تأكد من تعريف client في النطاق العام
let client;
...
// دالة للبحث عن التغريدات المتعلقة بالتكوين
async function searchTweets(token) {
  const url = `https://x.com/search?q=${token}&src=typed_query&f=live`;
...
// دالة لإدارة التكوينات
async function manageToken(client, token) {
  console.log(`🔍 البحث عن التغريدات المتعلقة بالتكوين: ${token}`);

  const startTime = Date.now();
  let tweetCount = 0;

  while (Date.now() - startTime < 60 * 1000) { // دقيقة واحدة
    const tweets = await searchTweets(token);
    tweetCount += tweets.length;

    if (tweetCount >= 10) {
      console.log(`🔥 التكوين ${token} عليه Hype عالي! سيتم إرساله إلى بوت GMGN.`);
      await tradeInGMGNBot(client, token);
      return;
    }

    await sleep(5000); // انتظر 5 ثوانٍ قبل المحاولة مرة أخرى
  }

  console.log(`❌ التكوين ${token} لم يحصل على 10 تغريدات خلال دقيقة. سيتم تجاهله.`);
}

// تعديل في التوجيه لإضافة إدارة التكوينات
client.addEventHandler(async (update) => {
  try {
    if (update.message && typeof update.message.message === "string") {
      const msg = update.message;
      ...
      if (/counts:\s*1(\D|$)/.test(text)) {
        ...
        if (caMatch && caMatch[1]) {
          const token = caMatch[1];
          console.log(`🔑 تم استخراج التكوين: ${token}`);
          fs.writeFileSync('last_token.txt', token, 'utf8');

          // إدارة التكوين بدلاً من إرساله مباشرةً
          await manageToken(client, token);
        }
      }
    }
  } catch (err) {
